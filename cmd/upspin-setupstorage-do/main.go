...
const help = `
Setupstorage-do is the second step in establishing an upspinserver.
It sets up DigitalOcean storage for your Upspin installation. You may skip this step
...
Setupstorage-do updates the server configuration files in $where/$domain/ to use
...
func main() {
	const name = "setupstorage-do"

	log.SetFlags(0)
	log.SetPrefix("upspin setupstorage-do: ")
